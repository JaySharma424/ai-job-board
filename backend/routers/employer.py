...
def fast_clean_description(text: str) -> str:
    """Basic fallback cleaner for instant publishing"""
...
def build_job_text(job: dict) -> str:
    ...
    desc_text = str(job.get("description", ""))[:300].replace("\n", " ")
...
# ==========================================
# ⚡ 1. INSTANT JOB PUBLISHING ROUTE
# ==========================================
@router.post("/jobs")
async def post_employer_job(data: JobPostRequest):
    try:
        ...
        job_doc = {
            ...
            "formattedDescription": fast_clean_description(data.description),        
            "skills": data.skills,
            "via": "Direct Employer",
        ...
        return {"success": True, "job_id": unique_job_id, "message": "Job published instantly. AI Markdown enrichment scheduled."}
...
async def run_daily_etl(
    ...
    try:
        ...
        clean, structured, machine-readable JSON representation.

        IMPORTANT:
        - Extract information ONLY from the provided job description.
        - Do NOT invent, assume, or hallucinate skills, qualifications, or requirements.
        - Normalize equivalent terms (e.g., "Python programming" → "Python", "MS Excel" → "Excel").
        - Remove duplicate skills.
        - If experience is unavailable, use "Not Specified".

        FORMATTED DESCRIPTION RULES:
        - Rewrite the raw JD into clean, professional Markdown.
        - Use Markdown section headers in bold (e.g., **Responsibilities:**).
        - Use bullet points with "-".
        - Do NOT add information that does not exist in the original JD.
        - Do NOT use HTML, Markdown tables, or emojis.

        REQUIRED JSON SCHEMA FORMAT:
        You MUST return a single JSON object where the keys are the `job_id` and the values are the extracted data.
        Return ONLY valid JSON. No backticks or code blocks.
        ...
        {{
            "job_id_1": {{
                ...
                "formatted_description": "**About the Role:**\\nWe are looking for...\\n\\n**Responsibilities:**\\n- Duty 1"
            }}
        }}

        BATCH JOBS TO PROCESS:
